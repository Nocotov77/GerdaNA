/**
 * ЛАБОРАТОРНАЯ РАБОТА 6. ЧАСТЬ 3 - JAVASCRIPT И ФУНКЦИОНАЛЬНОЕ ПРОГРАММИРОВАНИЕ
 * Полная реализация всех концепций в одном файле
 */
...
console.log('\n' + '='.repeat(70));
console.log('КРАТКИЕ ОТВЕТЫ НА КОНТРОЛЬНЫЕ ВОПРОСЫ');
console.log('='.repeat(70));

console.log(`
1️⃣  Методы массивов как функции высшего порядка:
   ...
   • some() / every() - проверяют условие

2️⃣  Замыкания в JavaScript:
   • Функция запоминает переменные из своего окружения
   • Используются для: инкапсуляции данных, создания счётчиков
   • Ключевое слово 'nonlocal' не нужно (работает автоматически)

3️⃣  Иммутабельность:
   • Создание новых объектов/массивов вместо изменения старых
   • Преимущества: проще отследить изменения, избегаем побочных эффектов
   • Инструменты: spread оператор (...), map(), filter(), slice()

4️⃣  Каррирование и функциональная композиция:
   • Каррирование: преобразование f(a,b,c) в f(a)(b)(c)
   • Композиция: объединение простых функций в сложные
   • Позволяет создавать специализированные функции

5️⃣  Асинхронное программирование:
   • Promises и async/await в функциональном стиле
   • Цепочки async функций (compose, pipe)
